...
@section('main')
	...
	<div class="panelContent container col-xs-8">
		<div class="col-md-6">
			<h4>Edit Measure Type in Analyzer Type</h4>
			{{ Former::open()->class('form')->method('POST')->enctype('multipart/form-data')->action(URL::route('postEditMeasureTypeInAnalyzer', ['mid' => $measureTypeInAnalyzer->id]))
				->rules([
				...
				{{Former::populate($measureTypeInAnalyzer)}}
				...
				{{Former::select('active')->options([1=>'Active', 0=>'Inactive'])->label('Active(for measure type in analyzer type)')->class('form-control')}}
				...
				<div class="control-group required">
					...
					@foreach ($analyzerTypes as $analyzerType)
						<option {{$measureTypeInAnalyzer->analyzer_types_id == $analyzerType['id'] ? 'selected': ''}}  value="{{$analyzerType['id']}}">{{$analyzerType['name']}}</option>
					@endforeach
					</select>
				</div>
